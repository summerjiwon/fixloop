...
export default function DemoPage() {
  return <main className="min-h-screen bg-mist px-5 py-8 text-ink lg:px-8"><div className="mx-auto max-w-5xl"><header className="flex flex-wrap items-start justify-between gap-4"><div><div className="flex items-center gap-2"><img src="/brand-mark.png" alt="" className="size-8 object-contain" /><p className="text-xs font-bold text-brand-700">공간기록 · AI FLOW</p></div><h1 className="mt-2 text-3xl font-bold tracking-tight">사진 신고부터 조치 검증까지</h1><p className="mt-2 text-sm leading-6 text-slate-600">로그인 없이 신고, AI 분석 결과, 운영 흐름까지 바로 체험할 수 있습니다.</p></div><Link href="/report/00000000-0000-0000-0000-000000000001" className="rounded-xl bg-brand-600 px-4 py-3 text-sm font-bold text-white">1. 직접 사진 신고 체험</Link></header>
    ...
    <section className="mt-5 rounded-2xl border border-brand-100 bg-white p-6 shadow-card"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-bold text-brand-700">심사용 공개 체험</p><h2 className="mt-2 text-xl font-bold">로그인 없이 전체 흐름을 확인하세요</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">실제 신고 접수와 개인 신고 확인은 공개로 제공하고, 운영 화면은 개인정보나 실제 데이터를 바꾸지 않는 읽기 전용 예시로 제공합니다.</p></div><div className="flex flex-wrap gap-2"><Link href="/report-status/0a677a37-676a-4543-94bf-0dde10893afe" className="rounded-xl border border-slate-200 px-4 py-3 text-sm font-bold text-ink">2. 신고 확인 보기</Link><Link href="/demo/operations" className="rounded-xl bg-brand-600 px-4 py-3 text-sm font-bold text-white">3. 운영 화면 보기</Link></div></div></section>
